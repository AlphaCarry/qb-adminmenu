...
RegisterNetEvent('qb-admin:client:SaveCar', function()
    ...
    if veh ~= nil and veh ~= 0 then
        ...
        if QBCore.Shared.Vehicles[vehname] ~= nil and next(QBCore.Shared.Vehicles[vehname]) ~= nil then
            ...
            QBCore.Functions.Notify(Lang:t("error.no_store_vehicle_garage"), 'info')
        end
    else
...
RegisterNetEvent('qb-weapons:client:SetWeaponAmmoManual', function(weapon, ammo)
    ...
    if weapon ~= "current" then
        ...
        if weapon ~= nil then
            ...
            QBCore.Functions.Notify(Lang:t("error.no_weapon"), 'info')
        end
    end
end)
...
function PerformanceUpgradeVehicle(vehicle, customWheels)
    ...
    if DoesEntityExist(vehicle) and IsEntityAVehicle(vehicle) then
        ...
        ToggleVehicleMod(vehicle, 11, true) -- Engine
        ToggleVehicleMod(vehicle, 12, true) -- Brake
        ToggleVehicleMod(vehicle, 13, true) -- Transmission
        ToggleVehicleMod(vehicle, 15, true) -- Suspension
